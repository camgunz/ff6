...
IDENTIFIER_REGEX = re.compile('\W|^(?=\d)')
    ...
    def __getattr__(self, attr_name):
        ...
        if index is None:
            for index, element in enumerate(self):
                ...
                raise AttributeError(f"{type(self).__name__} has no attribute '{attr_name}'")
        return self[index]

    def __setattr__(self, attr_name, attr):
        if not '_NamedElementList__initialized' in self.__dict__:
